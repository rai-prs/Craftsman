class DiagnosesController < ApplicationController
  ...
  def show
    ...
    @craftsmanship = (@diagnosis.communication.to_i + @diagnosis.planning.to_i + @diagnosis.dexterous.to_i +
                      @diagnosis.tidy.to_i + @diagnosis.physically.to_i + @diagnosis.muscle.to_i + @diagnosis.intelligence.to_i +
                      @diagnosis.pride.to_i + @diagnosis.picky.to_i) * 2.4 - @diagnosis.destruction.to_i * 3 -
                      @diagnosis.narrow.to_i - @diagnosis.dirt.to_i - @diagnosis.high.to_i - @diagnosis.careful.to_i * 2
  ...
  def diagnosis_params
    params.require(:diagnosis).permit(:id, :communication, :planning, :dexterous, :tidy, :physically, :muscle, :intelligence,
                                      :careful, :pride, :picky, :narrow, :dirt, :destruction, :high, :user_id)
  end
end
